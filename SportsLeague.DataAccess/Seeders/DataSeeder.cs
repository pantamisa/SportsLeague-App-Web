...
public static class DataSeeder
{
    ...
    {
        // Guardia de ejecución condicional: si ya hay equipos, no hacer nada
        // Esta línea es lo que impide duplicar datos si se reinicia la app
        ...
        // SaveChangesAsync aquí es crítico: genera los Ids en BD antes de asignarlos
        // a los jugadores en el siguiente bloque. Sin este save, teams[i].Id sería 0.

        // ═══ 2. JUGADORES — 4 por equipo, 80 en total ═══
        // Se usa un array de tuplas para mantener los datos compactos y legibles.
        // El orden del array externo coincide exactamente con el orden de teams arriba.
        ...
        for (int i = 0; i < teams.Count; i++)
        {
            foreach (var pd in playersData[i])
            {
                players.Add(new Player
                {
                    ...
                    Number    = pd.Number,
                    Position  = pd.Pos,
                    // BirthDate escalonada para que no todos tengan la misma fecha
                    BirthDate = new DateTime(1995, 1, 1).AddMonths(players.Count),
                    TeamId    = teams[i].Id  // teams[i].Id ya tiene valor porque se hizo SaveChangesAsync antes
                });
            }
        }
        ...
        var tournament = new Tournament
        {
            ...
            Status    = TournamentStatus.InProgress  // Listo para crear partidos sin pasos extra
        };
        context.Tournaments.Add(tournament);
        await context.SaveChangesAsync();

        // ═══ 5. INSCRIBIR LOS 20 EQUIPOS EN EL TORNEO ═══
        ...
        // ═══ 6. PARTIDO DE PRUEBA (Nacional vs Medellín) ═══
        var match = new Match
        {
            TournamentId = tournament.Id,
            HomeTeamId   = teams[0].Id, // Nacional
            AwayTeamId   = teams[1].Id, // Medellín
        ...
        // ═══ 7. ALINEACIONES (11 por equipo) ═══
        // Nota: Como el seeder solo crea 4 jugadores por equipo, alinearemos esos 4 
        // para demostrar la funcionalidad sin romper el límite de 11.
        
        // Jugadores de Nacional (IDs 1-4 en un inicio limpio)
        var nationalPlayers = await context.Players.Where(p => p.TeamId == teams[0].Id).ToListAsync();
        foreach (var p in nationalPlayers)
        {
            context.MatchLineups.Add(new MatchLineup
            {
                ...
                PlayerId = p.Id,
                IsStarter = true,
                Position = p.Position.ToString().Substring(0, 2).ToUpper()
            });
        }

        // Jugadores de Medellín (IDs 5-8)
        var medellinPlayers = await context.Players.Where(p => p.TeamId == teams[1].Id).ToListAsync();
        foreach (var p in medellinPlayers)
        {
            context.MatchLineups.Add(new MatchLineup
            {
                MatchId = match.Id,
